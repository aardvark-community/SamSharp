...
module CameraController =
    ...
    let rec update (model : CameraModel) (message : Message) =
        ...
        | Resize size when size <> model.viewportSize ->
            if model.viewportSize.AnyGreater 1 then
                let bounds = model.target.bounds(model.viewportSize)
                let state = CenterAndScale.FromBounds(bounds, size)
                (withCamera << clampPosition)
                    { model with
                        target = state
                        current = state
                        viewportSize = size
    ...
    let attributes (env : Env<Message>) (shouldPan : MouseEvent -> bool) (shouldZoom : MouseEvent -> bool) (m : aval<AdaptiveCameraModel>) =
        ...
        att {
            ...
            panning |> AVal.map (function
                | true ->
                    Dom.OnPointerMove (fun e ->
                        env.Emit [Move(V2d e.ClientPosition)]
                    ) |> Some
                | _ ->
                    None
            )
